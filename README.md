...
一款专为2026年设计的精致饮酒记录工具，四宫格日历、多人状态追踪、云端同步，完美记录与好友共饮的每一刻。
...
- 📅 **2026年专属日历** — 全年月历视图，每天四宫格清晰展示四位成员的饮酒状态
- 🧑‍🤝‍🧑 **四人团队记录** — 支持四名成员（可自定义姓名、颜色、简称），固定四角布局
...
### 1. 下载或克隆本仓库
```bash
git clone https://github.com/yourname/drink-calendar-2026.git
```
...
- **日历记录**：以 `年份` → `月份` → `日期` 为层级，存储每位成员的状态（通过用户 ID 关联）及当日照片（Base64）。
...
- 最多支持 **4 人**，每人固定一个宫格位置（左上、右上、左下、右下）。
